...
router.get('/username/:username',IsLoggedIn, async function(req, res) {
  const reg=new RegExp(`^${req.params.username}`,`i`)
  const users=await userModel.find({username:reg})
  res.json(users)
});
...
router.post("/update",upload.single("pimage"),async function(req,res,next){
 let user=await userModel.findOneAndUpdate({username:req.session.passport.user},
